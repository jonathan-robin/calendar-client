...
function Month(props: {handleClickMonthSelectedDate: any; todos: Todo[] | undefined; getTodos: any;}) {
  ...
  const getFullMonth = (firstDayOfTheMonth: any) => {
    ...
    if (
      firstDayOfTheMonth.getMonth() % 2 !== 0 &&
      firstDayOfTheMonth.getMonth() !== 1 &&
      firstDayOfTheMonth.getMonth() < 7
    ) {
      numberDay = 30;
    ...
    for (var i = 1; i < numberDay; i++) {
      fullMonth.push(
        new Date(firstDayOfTheMonth.getTime() + 3600 * 1000 * 24 * i)
      );
    }
  };
  ...
  return (
    <div className="calendar__background cb_month">
      <div className="switchWeek stress">
        ...
        {monthString.toLocaleDateString("fr-FR", { month: "long" })}
              ...
              <div className="calendar-month--day ">
                ...
                >
                  <div
                    ...
                    id={day.toString()}
                    onClick={(event) =>
                      props.handleClickMonthSelectedDate(event.currentTarget.id)
                    }
                  >
                  ...
                  <div className="todo">
                    <div className="todo-content todo-content-week regular">
                      {todos?.map((todo, index) => {
                        let time;
                        todo.startingTime > 9
                          ? (time = todo.startingTime.toString() + "h00")
                          : (time = "0" + todo.startingTime.toString() + "h00");
                        let nd = new Date(todo.day);
                        if (
                          day.getMonth() === nd.getMonth() &&
                          day.getFullYear() === nd.getFullYear() &&
                          day.getDate() === nd.getDate()
                        ) {
                          return (
                            <div className="todo-content-week-text">
                              <div className="todo-content-week-time">
                                {time}
                              </div>
                              <div className="todo-content-week-content">
                                {todo.content}
                              </div>
                            </div>
                          );
